...
/*   Updated: 2021/05/15 13:50:42 by timotheein       ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	send_to_b(t_all *all)
{
	...
	while (all->sk_a && !(is_sk_ordered(all->sk_a, 1) && !all->sk_b))
	{
		...
		while ((e = all->sk_a) && in_packet(e, all->v_nxtp)
		&& ft_lstsize_e(all->sk_a) > 0)
			while_send_b(all, e);
		if (all->sk_b && val(ft_lstlast_e(all->sk_b)) == all->v_nxtp)
			set_pivot_at_top_b(all);
		if (is_sk_ordered(e, -1) && is_all_end_sup(e, all->v_nxtp))
			return ;
